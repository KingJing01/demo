...
func UpdateTenantById(m *Tenant, sysCode string, perIdStr string, perMenu string, tenId int64, userID int64) (err error) {
	...
	var maps []orm.Params
	o.Raw("select t1.UserId from userrole t1 where t1.TenantId = ? and t1.RoleId=1 and t1.SysCode=?", tenId, sysCode).Values(&maps)
...
func GetPerInfoForTenant(sysCode string, tenantId int64) (result []out.PermissionCheckInfo, err error) {
	...
	FROM (SELECT 	MenuCode,	DisplayName,NAME FROM	permission WHERE SysCode = ? AND TenantId = ?  and IsMenu = 1 ) t1
	...
	GROUP BY 	t1. NAME ) t5 GROUP BY NAME`, sysCode, tenantId, tenantId).QueryRows(&result)
	return result, err
}
