...
func GetPermBySetMealCode(setMeatCode string, sysCode string) (result []out.PermissionCheckInfo, err error) {
	...
	_, err = o.Raw(`SELECT t3.NAME name,t3.DisplayName display_name ,GROUP_CONCAT(t1. NAME) code,GROUP_CONCAT(t1.DisplayName) code_name,
		...
		permission WHERE  isMenu = 1  and TenantId=0 ) t1 LEFT JOIN ( SELECT 	PermissionCode FROM permissionpackage WHERE SetMealCode = ? ) t2 ON t1. NAME = t2.PermissionCode
	LEFT JOIN (SELECT NAME,DisplayName,MenuCode FROM permission	WHERE  isMenu = 0) t3 on t1.MenuCode = t3.MenuCode
	GROUP BY t1.MenuCode order by t1.MenuCode,t1.Id asc`, setMeatCode).QueryRows(&result)
	return result, err
}
